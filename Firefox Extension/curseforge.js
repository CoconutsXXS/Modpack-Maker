...
(async () =>
{while(true){if(document.getElementById('instance-download')!=undefined && document.querySelector("aside > div > div:nth-child(3) > div > div > button")!=undefined){await new Promise(resolve=>setTimeout(resolve,100));continue;}
            ...
            c.querySelector(".split-button").firstChild.onclick = async (e) =>
            {
                ...
                switch(allFiles[0].gameVersions.findLast(g=>!minecraftVersionReg.test(g)&&g.toLowerCase()!="server"&&g.toLowerCase()!="client").toLowerCase())
                {
                    ...
                    case 'forge':
                    {
                        ...
                        for(let v of data.metadata.versioning.versions.version.filter(e => e["#text"].split('-')[0] == allFiles[0].gameVersions.findLast(g=>minecraftVersionReg.test(g)).toString()))
                        {
                    ...
                    case 'neoforge':
                    {
                        ...
                        for(let v of data.metadata.versioning.versions.version.filter(e => '1.'+e["#text"].split('-')[0].substring(0, e["#text"].split('-')[0].lastIndexOf('.')) == allFiles[0].gameVersions.findLast(g=>minecraftVersionReg.test(g)).toString()).reverse())
                        {
