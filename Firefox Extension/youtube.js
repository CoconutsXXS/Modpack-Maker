...
if(window.electron)
{
    externalFunctions =
    {
        fetchToText: async (url) =>
        {
            return await window.electron.fetchTxt(url)
        },
        openMod: async (url) =>
        {
            await window.electron.sendToHost('open-mod', url);
        },
        save: (data) =>
        {
            window.electron.sendToHost('save', data);
        },
        unsave: (url) =>
        {
        ...
        isSaved: (url) =>
        {
            ...
            {
                window.isSaved = (r) => {resolve(r)}
                window.electron.sendToHost('isSaved', url)
            })
        }
    }
}
else
{
    externalFunctions =
    {
        ...
        isSaved: async (url) =>
        {
            return (await browser.runtime.sendMessage({ action: "app", content:
            {
                name: "isSaved",
                value: url
            }})).result
...
history.replaceState = function(state, title, url)
{
    ...
    console.log('moving to:',futurUrl)
...
function setup()
{
    ...
    {
        escapeHTMLPolicy = {createHTML: (arg) => arg}
    }
    ...
    async function handleDescription()
    {
        ...
        for(let e of linkText)
        {
            ...
            {
                let doc = await externalFunctions.fetchToText(`https://docs.google.com/document/d/${url.pathname.split('/')[3]}/export?format=txt`);
            ...
            {
                let doc = new DOMParser().parseFromString(escapeHTMLPolicy.createHTML(await externalFunctions.fetchToText(url.href)), "text/html");
    ...
    async function setCurrentMod(baseUrl)
    {
        ...
        {
            let i = [];
            let res = await externalFunctions.fetchToText(`https://www.curseforge.com/minecraft/${url.pathname.split('/')[2]}/${url.pathname.split('/')[3]}/gallery`);
        ...
        else
        {
            ...
            let res = await externalFunctions.fetchToText(`https://modrinth.com/${url.pathname.split('/')[1]}/${url.pathname.split('/')[2]}/gallery`);
            let galPage = new DOMParser().parseFromString(escapeHTMLPolicy.createHTML(res), "text/html");
...
document.addEventListener("DOMContentLoaded", () => {if(!isSetup){setup()}})
