...
window.setupExplorer = (directory = '', container, files, onMove = (from, to) => {}, onSelect = (f) => {}, deleteEvent=()=>{}, activationEvent=()=>{}, propertieName = null, onFile=(f)=>{}) =>
{
    ...
    function reloadExplorer(directory = [])
    {
        ...
        for(let d of directory)
        {
            ...
            for(let e of Object.values(hierarchy))
            {
                if(e.folder) { console.log(e); console.log(JSON.stringify(e.path)+' != '+JSON.stringify(progressionPath.filter(p=>p!='')), JSON.stringify(e.path) != JSON.stringify(progressionPath.filter(p=>p!=''))) }
                ...
                function up()
                {
                    ...
                    if(lastClosest!=null)
                    {
                        ...
                        if(!lastClosest.hasAttribute('folder'))
                        {
                        ...
                        function move(e, dest)
                        {
                            ...
                            {
                                hierarchy[e.name].path = dest;
                            }

                            if(e.folder && e.children!=undefined)
                            {
                                console.log('Moving folder -> moving children')
                                let children = Object.values(hierarchy).filter(o =>
                                {
                                    // console.log(JSON.stringify(o.path) == JSON.stringify((fromPath+e.name).split('/').filter(z=>z!='')))
                                    // console.log(JSON.stringify(o.path), JSON.stringify((fromPath+e.name).split('/').filter(z=>z!='')))
                                    console.log(JSON.stringify(o.path)+" == "+JSON.stringify((fromPath+e.name).split('/').filter(z=>z!='')), JSON.stringify(o.path) == JSON.stringify((fromPath+e.name).split('/').filter(z=>z!='')))
                                    return JSON.stringify(o.path) == JSON.stringify((fromPath+e.name).split('/').filter(z=>z!=''))
                                })
                            ...
                            onMove(fromPath+e.filename, path+e.filename, e.folder)
                        }
                ...
                document.addEventListener('mousemove', (e) =>
                {
                    ...
                    else
                    {
                        ...
                        b.style.top = buttonStart.y - (mouseStart.y-mousePosition.y) - b.parentNode.parentNode.getBoundingClientRect().y + b.getBoundingClientRect().height+ 'px';
                        b.style.left = buttonStart.x - (mouseStart.x-mousePosition.x) - b.parentNode.parentNode.getBoundingClientRect().x + 'px';
                        ...
                        // Hightlight
                        for(let c of closest[0].parentNode.parentNode.childNodes){c.removeAttribute('highlight')}
                        for(let c of closest){c.removeAttribute('highlight')}
                        if(closest[0].hasAttribute('folder'))
                        {
                            closest[0].setAttribute('highlight', '');
                        }
                        else
                        {
                            closest[0].parentNode.setAttribute('highlight', '');
                        }
...
window.loadModsExplorer = (directory = null) =>
{
    ...
    console.log(window.instance.virtualDirectories)
    ...
    window.setupExplorer(directory, document.getElementById('mods-explorer'), files, (from, to, folder = false) =>
    {
        console.log(from, to, folder)
        if(folder)
        {
            let fromPath = ''; if(from.includes('/')) { fromPath = from.slice(0, from.lastIndexOf('/')) }
            let toPath = ''; if(to.includes('/')) { toPath = to.slice(0, to.lastIndexOf('/')) }
            console.log(fromPath, toPath)
            window.instance.virtualDirectories[window.instance.virtualDirectories.findIndex(d=>d.parent=='mods'&&d.name == from.split('/')[from.split('/').length-1]&&d.path == fromPath)].path = toPath
