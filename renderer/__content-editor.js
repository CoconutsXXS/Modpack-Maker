...
let modEditor = document.getElementById("mod-content-explorer"); modEditor.style.display = "none";
...
async function jarAnalyseSetup(i)
{
    ...
    // Full Combinaison
    window.jarData.content = await JarContent.get(i.name, i.version.number)
    ...
    window.jarData.openExplorer = async function(path = "")
    {
        ...
        explorer(window.jarData.content.combined, async (keys) =>
        {
            ...
            const result = await window.jarData.content.getFile(keys, async files =>
                new Promise(resolve =>
                {
                    console.log(files)
            ...
            fileEditor(lodash.clone(result.value), keys[keys.length-1], (value) =>
            {
                if(lodash.isEqual(result.value, value)){return}
                
                setProp(window.jarData.content.combined, keys, value);
                if(window.jarData.modified.find(m => m.keys==keys))
                {
                    window.jarData.modified[window.jarData.modified.findIndex(m => m.keys==keys)] = {origin: result.jarFile, keys, value}
                }
                else
                {
                    window.jarData.modified.push({origin: result.jarFile, keys, value})
                }
            ...
            {
                return window.jarData.content.getFile(keys, files => files[files.length-1])
            }, (keys) =>
            {
                return window.jarData.content.getFile(keys, files => files[files.length-1])
            })
    ...
    window.jarData.loadTable = async function loadTable(force = false)
    {
        ...
        let dataCopy = lodash.clone(window.jarData.content.combined.data); Object.assign(dataCopy, window.jarData.content.combined.assets)
        let combinedEntrie = Object.entries(dataCopy).filter(([e,v]) => lodash.isObject(v) && e != ".mcassetsroot");
        for(let [e, v] of combinedEntrie)
        {
            if(window.jarData.content.combined.data[e])
            {
                v = lodash.clone(window.jarData.content.combined.data[e])
                Object.assign(v, window.jarData.content.combined.assets[e])
            }
            else
            {
                v = window.jarData.content.combined.assets[e];
            }
            ...
            async function recursiveSearch(p, preKeys = [])
            {
                for(let [key, v] of Object.entries(p))
                {
                    ...
                    if(key.includes("."))
                    {
                        ...
                        {
                            let isAsset = window.jarData.content.getFile(["assets", e, ...shortPath.split("/")])!=undefined;
...
{
    document.querySelector("#mod-content-explorer").style.display = "none";
    document.querySelector("#content-editor .expend").style.display = "block";
}
document.querySelector("#content-editor .expend").onclick = () =>
{
    document.querySelector("#mod-content-explorer").style.display = "flex";
...
function explorer(o, open, display, keysPath = [], iteration = 0, startingPath = [], max = 6)
{
    ...
    document.getElementById("mod-content-explorer").scrollTo(document.getElementById("mod-content-explorer").scrollHeight, 0)
}
...
{    
    ...
    else if(filename.endsWith(".nbt"))
    {
        ...
        editors.nbtEditor.querySelectorAll(":scope > button")[0].onclick = async () => 
        {
            ...
            let data =
            {
                parsed: await ipcInvoke("readFile", await ipcInvoke("appData")+sep()+'Modpack\ Maker'+sep()+'instances'+sep()+window.instance.name+sep()+'minecraft'+sep()+'saves'+sep()+'.Structure Edition'+sep()+'generated'+sep()+'minecraft'+sep()+'structures'+sep()+'structure'+(nbtEditorIndex==0?'':nbtEditorIndex)+'.nbt'),
...
window.openModContent = async function(path)
{
    ...
    explorer(data, async (keys) =>
    {
        ...
        const result = window.jarData.content.getFile(keys, files => files.find(f=>f.jarFile.endsWith(fullPath)))
        ...
        {
            selectedFile = {value: result, keys: keys, jarFile: fullPath};
        }
